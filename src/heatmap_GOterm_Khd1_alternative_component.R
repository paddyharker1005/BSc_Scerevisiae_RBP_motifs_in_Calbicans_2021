...
table_GOcomponent <- read_excel("GO_Khd1_all_motif_component.xlsx")

table_GOcomponent <- data.frame(table_GOcomponent)

component_heatmap <- ggplot(table_GOcomponent, aes(x = motif, y = GO_term, fill= minuslogP, las = 23)) + 
  ...
  labs(fill = "-log(P)", x = "Khd1 motif", y = "Component") +
