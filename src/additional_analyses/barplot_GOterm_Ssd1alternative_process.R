...
table_GOprocess_Ssd1 <- read_excel("GO_term_Ssd1_alternative_process.xlsx")

ggplot(table_GOprocess_Ssd1, aes(x= reorder(GO_term, minuslogP), y= minuslogP)) +
  geom_point(colour = 'blue') +
  geom_segment(aes(x=GO_term, xend=GO_term, y=0, yend=minuslogP), colour = 'blue') +
  scale_y_continuous(limits = c(2,8), oob = rescale_none) +
