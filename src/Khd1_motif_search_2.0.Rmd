...
arrange(motif_count_up_df,desc(count_up200)) %>%
    head(n = 20) %>%
    knitr::kable()
```
...
arrange(motif_count_down_df,desc(count_down200)) %>%
    head(n = 20) %>%
    knitr::kable()
```
...
id_file_HWNCATTWY1_up200 <- paste(here::here(), "results", "Ca_id_list_HWNCATTWY1_up200.txt", sep = "/")
motif_count_up_df %>%
    filter(count_up200 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_HWNCATTWY1_up200)
id_file_HWNCATTWY2_up200 <- paste(here::here(), "results", "Ca_id_list_HWNCATTWY2_up200.txt", sep = "/")
motif_count_up_df %>%
    filter(count_up200 >= 2) %>%
    pull(id) %>%
    write_lines(file = id_file_HWNCATTWY2_up200)
```

```{r gene_list_output_down}
id_file_HWNCATTWY1_down200 <- paste(here::here(), "results", "Ca_id_list_HWNCATTWY1_down200.txt", sep = "/")
motif_count_down_df %>%
    filter(count_down200 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_HWNCATTWY1_down200)
id_file_HWNCATTWY2_down200 <- paste(here::here(), "results", "Ca_id_list_HWNCATTWY2_down200.txt", sep = "/")
motif_count_down_df %>%
