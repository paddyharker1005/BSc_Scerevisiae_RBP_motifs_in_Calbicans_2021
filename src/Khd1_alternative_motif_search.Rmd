---
title: "Scerevisiae Khd1 alternative broad motif search in Calbicans"
author: "Paddy"
date: "24/02/2021"
output: html_document
---
## Summary
We are looking for occurrences of the S. cerevisiae Khd1 motif (CNN)6 from Hasegawa et al. (2008) in the CDS and UTRs of C. albicans genes to form hypotheses about homologous post-transcriptional regulation.
...
motif_count_up_df <- tibble(id = Ca_names_up_info$id,
                            ...
                            count_up1000 = vcountPattern(pattern = DNAString("CNNCNNCNNCNNCNNCNN"),
                                 subject = Ca_up1000_DSS,
                                 fixed = "subject"),
                            count_up200 = vcountPattern(pattern = DNAString("CNNCNNCNNCNNCNNCNN"),
                                 subject = subseq(Ca_up1000_DSS,start = 801L, end = 1000L),
                                 fixed = "subject"),
                            count_up100 = vcountPattern(pattern = DNAString("CNNCNNCNNCNNCNNCNN"),
...
motif_count_down_df <- tibble(id = Ca_names_down_info$id,
                            ...
                            count_down1000 = vcountPattern(pattern = DNAString("CNNCNNCNNCNNCNNCNN"),
                                 subject = Ca_down1000_DSS,
                                 fixed = "subject"),
                            count_down200 = vcountPattern(pattern = DNAString("CNNCNNCNNCNNCNNCNN"),
                                 subject = subseq(Ca_down1000_DSS,start = 1L, end = 200L),
                                 fixed = "subject"),
                            count_down100 = vcountPattern(pattern = DNAString("CNNCNNCNNCNNCNNCNN"),
...
id_file_CNN1_up100 <- paste(here::here(), "results", "Motif Searches", "Khd1 Alternative", "Ca_id_list_(CNN)1_up100.txt", sep = "/")
motif_count_up_df %>%
    filter(count_up100 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_CNN1_up100)
id_file_CNN2_up100 <- paste(here::here(), "results", "Motif Searches", "Khd1 Alternative", "Ca_id_list_(CNN)2_up100.txt", sep = "/")
motif_count_up_df %>%
    filter(count_up100 >= 2) %>%
    pull(id) %>%
    write_lines(file = id_file_CNN2_up100)
```
```{r gene_list_output_CDS}
id_file_CNN1_CDS <- paste(here::here(), "results", "Motif Searches", "Khd1 Alternative", "Ca_id_list_(CNN)1_CDS.txt", sep = "/")
motif_count_CDS_df %>% 
  filter(count_CDS >= 1) %>% 
  pull(id) %>% 
  write_lines(file = id_file_CNN1_CDS)
id_file_CNN2_CDS <- paste(here::here(), "results", "Motif Searches", "Khd1 Alternative", "Ca_id_list_(CNN)1_CDS.txt", sep = "/")
motif_count_CDS_df %>% 
  filter(count_CDS >= 2) %>% 
  pull(id) %>%
  write_lines(file = id_file_CNN2_CDS)
```
```{r gene_list_output_down}
id_file_CNN1_down200 <- paste(here::here(), "results", "Motif Searches", "Khd1 Alternative", "Ca_id_list_(CNN)1_down200.txt", sep = "/")
motif_count_down_df %>%
    filter(count_down200 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_CNN1_down200)
id_file_CNN2_down200 <- paste(here::here(), "results", "Motif Searches", "Khd1 Alternative", "Ca_id_list_(CNN)2_down200.txt", sep = "/")
motif_count_down_df %>%
    filter(count_down200 >= 2) %>%
    pull(id) %>%
    write_lines(file = id_file_CNN2_down200)
```