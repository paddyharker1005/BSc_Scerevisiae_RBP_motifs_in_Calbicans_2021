...
table_GOcomponent_Ssd1 <- read_excel("GO_term_Ssd1_alternative_component.xlsx")

ggplot(table_GOcomponent_Ssd1, aes(x= reorder(GO_term, minuslogP), y= minuslogP)) +
  geom_point(colour = 'red') +
  geom_segment(aes(x=GO_term, xend=GO_term, y=0, yend=minuslogP), colour = 'red') +
  scale_y_continuous(limits = c(2,10), oob = rescale_none) +
