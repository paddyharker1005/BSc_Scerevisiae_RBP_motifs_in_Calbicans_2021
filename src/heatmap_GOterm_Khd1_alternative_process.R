...
table_GOprocess <- read_excel("GO_Khd1_all_motif_process.xlsx")

table_GOprocess <- data.frame(table_GOprocess)

process_heatmap <- ggplot(table_GOprocess, aes(x = motif, y = GO_term, fill= minuslogP, las = 23)) + 
  ...
  labs(fill = "-log(P)", x = "Khd1 motif", y = "Process") +
