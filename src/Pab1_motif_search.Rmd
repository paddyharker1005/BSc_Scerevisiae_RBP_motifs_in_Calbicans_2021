---
title: "Scerevisiae Pab1 broad motif search in Calbicans"
author: "Paddy"
date: "28/02/2021"
output: html_document
---
## Summary
We are looking for occurrences of the S. cerevisiae Pab1 motif in the CDS and UTRs of C. albicans genes to form hypotheses about homologous post-transcriptional regulation.
...
motif_count_up_df <- tibble(id = Ca_names_up_info$id,
                            ...
                            count_up1000 = vcountPattern(pattern = DNAString("WTATATAW"),
                                 subject = Ca_up1000_DSS,
                                 fixed = "subject"),
                            count_up200 = vcountPattern(pattern = DNAString("WTATATAW"),
                                 subject = subseq(Ca_up1000_DSS,start = 801L, end = 1000L),
                                 fixed = "subject"),
                            count_up100 = vcountPattern(pattern = DNAString("WTATATAW"),
...
motif_count_CDS_df <- tibble(id = Ca_names_CDS_info$id,
                            ...
                            count_CDS = vcountPattern(pattern = DNAString("WTATATAW"),
                                 subject = Ca_CDS_DSS,
                                 fixed = "subject"))
...
motif_count_down_df <- tibble(id = Ca_names_down_info$id,
                            ...
                            count_down1000 = vcountPattern(pattern = DNAString("WTATATAW"),
                                 subject = Ca_down1000_DSS,
                                 fixed = "subject"),
                            count_down200 = vcountPattern(pattern = DNAString("WTATATAW"),
                                 subject = subseq(Ca_down1000_DSS,start = 1L, end = 200L),
                                 fixed = "subject"),
                            count_down100 = vcountPattern(pattern = DNAString("WTATATAW"),
...
id_file_WTATATAW1_up100 <- paste(here::here(), "results", "Motif Searches", "Pab1", "Ca_id_list_WTATATAW1_up100.txt", sep = "/")
motif_count_up_df %>%
    filter(count_up100 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_WTATATAW1_up100)
id_file_WTATATAW2_up100 <- paste(here::here(), "results", "Motif Searches", "Pab1", "Ca_id_list_WTATATAW2_up100.txt", sep = "/")
motif_count_up_df %>%
    filter(count_up100 >= 2) %>%
    pull(id) %>%
    write_lines(file = id_file_WTATATAW2_up100)
```
```{r gene_list_output_CDS}
id_file_WTATATAW1_CDS <- paste(here::here(), "results", "Motif Searches", "Pab1", "Ca_id_list_WTATATAW1_CDS.txt", sep = "/")
motif_count_CDS_df %>% 
  filter(count_CDS >= 1) %>% 
  pull(id) %>% 
  write_lines(file = id_file_WTATATAW1_CDS)
id_file_WTATATAW2_CDS <- paste(here::here(), "results", "Motif Searches", "Pab1", "Ca_id_list_WTATATAW2_CDS.txt", sep = "/")
motif_count_CDS_df %>% 
  filter(count_CDS >= 2) %>% 
  pull(id) %>%
  write_lines(file = id_file_WTATATAW2_CDS)
```
```{r gene_list_output_down}
id_file_WTATATAW1_down200 <- paste(here::here(), "results", "Motif Searches", "Pab1", "Ca_id_list_WTATATAW1_down200.txt", sep = "/")
motif_count_down_df %>%
    filter(count_down200 >= 1) %>%
    pull(id) %>%
    write_lines(file = id_file_WTATATAW1_down200)
id_file_WTATATAW2_down200 <- paste(here::here(), "results", "Motif Searches", "Pab1", "Ca_id_list_WTATATAW2_down200.txt", sep = "/")
motif_count_down_df %>%
    filter(count_down200 >= 2) %>%
    pull(id) %>%
    write_lines(file = id_file_WTATATAW2_down200)
```